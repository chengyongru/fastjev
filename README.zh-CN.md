...
agent 工作流中的许多决策范围很窄：*路由到哪里*、*是否重试*、*证据是否支持 X*。常规生成路径可以回答这些问题，但它会输出文本，应用必须先解析才能执行分支。

[Jev](https://typesafe.ai/) 是 TypeSafe 面向运行时类型化决策的托管 System One Model。fastjev 使用开放模型实现其文档化的**接口模式**；它不复现 Jev 未公开的模型、训练方法、校准能力或性能。
...
`FastJev` 只依赖 `ScoringBackend` 协议。内置 Torch、MLX、llama.cpp GGUF 和可选 vLLM 实现都是适配器，因此切换 runtime 无需改变 `Choice`、`Boolean`、`Score` 或结果类型。通过 vLLM 批量进行 CUDA 推理时安装 `.[vllm]`；使用本地或 Hugging Face 托管的 GGUF 文件时安装 `.[llama-cpp]`。安装方式、batching、结果语义、backend 合约和 System One 适配方式见 [Python SDK 指南](docs/SDK.zh-CN.md)。
