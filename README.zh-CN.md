...
Qwen3.5-4B Q4_K_M 在 WSL2 上的模型加载耗时为 4.05 秒，三项 shell safeguard
决策的顺序执行中位耗时为 2.344 秒（1.28 次决策/秒）。三项结果都符合预期，输出
token 数均为 0。
...
llama.cpp 后端按顺序执行 `decide_many`；质量对比也没有将量化影响与 serving shape
影响分离。[结果文档](docs/RESULTS.zh-CN.md#rtx-5090-上的-llamacpp-q4_k_m)记录了精确条件、
逐行证据和声明边界。
